"""One-shot frozen Candidate-8 official Checkpoint-A runner."""
...
try:
    ...
    from .checkpoint_a_candidate8_prereg import ARTIFACT_NAMESPACE,CRITERIA,FROZEN_SOURCE,load,verify
except ImportError:
    from checkpoint_a_candidate6 import _counts_from_rows,_metrics,_write_json,canonical_sha256
    from checkpoint_a_candidate8_prereg import ARTIFACT_NAMESPACE,CRITERIA,FROZEN_SOURCE,load,verify
...
def verify_readiness(value,prereg):
    ...
    if value.get("receipt_sha256")!=canonical_sha256(unsigned) or not (value.get("schema_version")=="A.CANDIDATE8.PROVIDER.READINESS.1" and value.get("candidate")=="A-CANDIDATE-8" and value.get("frozen_semantic_source_revision")==FROZEN_SOURCE and value.get("preregistration_sha256")==prereg["preregistration_sha256"] and value.get("healthy") is True and value.get("benchmark_cases_used")==0): raise ValueError("passing Candidate-8 provider readiness receipt required")
def supervisor_head(): return subprocess.check_output(["git","rev-parse","HEAD"],cwd=ROOT,text=True).strip()

def main():
    p=argparse.ArgumentParser(); p.add_argument("--candidate-root",type=Path,required=True); p.add_argument("--preregistration",type=Path,required=True); p.add_argument("--binding",type=Path,required=True); p.add_argument("--summary",type=Path,required=True); p.add_argument("--receipt",type=Path,required=True); p.add_argument("--provider-readiness",type=Path); p.add_argument("--output-dir",type=Path); p.add_argument("--evidence-reference"); p.add_argument("--verify-offline",action="store_true"); a=p.parse_args()
    ...
    for index,gold in enumerate(frozen):
        result=rt["run_candidate8"](gold.instruction,provider); passed=False; detail=None
    ...
    decision={"candidate":"A-CANDIDATE-8","status":status,"processed":counts.processed,"provider_attempt_rows":len(rows),"mathematically_eliminated_after_case":eliminated,"metrics":metrics,"criteria":CRITERIA,"score_recovery_retries":0}; decision["decision_sha256"]=canonical_sha256(decision); _write_json(out/"decision.json",decision)
    aggregate={"candidate":"A-CANDIDATE-8","status":status,"processed":counts.processed,"total":80,"metrics":metrics,"decision_sha256":decision["decision_sha256"],"row_sha256":[r["row_sha256"] for r in rows]}; aggregate["aggregate_sha256"]=canonical_sha256(aggregate); _write_json(out/"aggregate.json",aggregate)
    if passed:
        spec=importlib.util.spec_from_file_location("c8_a_receipt",ROOT/"src/ecocommit/checkpoint_a_evidence.py"); module=importlib.util.module_from_spec(spec); sys.modules[spec.name]=module; spec.loader.exec_module(module); q=prereg["qualification"]
        typed=module.CheckpointAEvidenceReceipt(verification_mode="FROZEN_AGGREGATE",evidence_reference=a.evidence_reference,aggregate_sha256=aggregate["aggregate_sha256"],manifest_sha256=prereg["preregistration_sha256"],source_revision=prereg["supervisor_source_revision"],candidate_version="A-CANDIDATE-8",dataset_sha256=prereg["frozen_dataset"]["sha256"],total_cases=80,full_frozen_gate_run=True,gate_passed=True,metrics=module.CheckpointAMetrics(passed_cases=counts.case_passes,**metrics),candidate_source_revision=FROZEN_SOURCE,qualification_status="PASS",qualification_evidence_sha256=q["evidence_sha256"],qualification_source_revision=FROZEN_SOURCE,preregistration_sha256=prereg["preregistration_sha256"],evaluator_sha256=prereg["frozen_evaluator_sha256"],criteria_sha256=prereg["criteria_sha256"],artifact_namespace=ARTIFACT_NAMESPACE,semantic_score_retry_count=0); _write_json(out/"checkpoint-a-pass-receipt.json",typed.model_dump(mode="json"))
