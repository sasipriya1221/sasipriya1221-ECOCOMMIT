...
CANDIDATE8_FROZEN_SOURCE_REVISION = "850a7b5f1f21d7951cd4a9a840c0bebbb635d594"
CANDIDATE8_ARTIFACT_NAMESPACE = "checkpoint-a-candidate-8"
CANDIDATE8_EVALUATOR_SHA256 = CANDIDATE7_EVALUATOR_SHA256
CANDIDATE8_CRITERIA_SHA256 = CANDIDATE7_CRITERIA_SHA256
...
class CheckpointAEvidenceReceipt(BaseModel):
    ...
    def passed_receipt_is_coherent(self):
        if self.verification_mode == "FROZEN_AGGREGATE":
            if self.candidate_version not in {"A-CANDIDATE-3", "A-CANDIDATE-4", "A-CANDIDATE-5", "A-CANDIDATE-6", "A-CANDIDATE-7", "A-CANDIDATE-8"}:
        ...
        if self.candidate_version == "A-CANDIDATE-8":
            required = (self.qualification_evidence_sha256, self.preregistration_sha256, self.evaluator_sha256, self.criteria_sha256)
            if (
                self.candidate_source_revision != CANDIDATE8_FROZEN_SOURCE_REVISION
                or self.qualification_source_revision != CANDIDATE8_FROZEN_SOURCE_REVISION
                or self.qualification_status != "PASS"
                or self.artifact_namespace != CANDIDATE8_ARTIFACT_NAMESPACE
                or self.evaluator_sha256 != CANDIDATE8_EVALUATOR_SHA256
                or self.criteria_sha256 != CANDIDATE8_CRITERIA_SHA256
                or self.semantic_score_retry_count != 0
                or any(value is None for value in required)
            ):
                raise ValueError("Candidate-8 receipt lacks its frozen qualification/protocol binding")
