...
| Provider ambiguity | Exact provider bindings, complete redacted per-attempt retry chronology, provider/local idempotency, durable result recovery, capture re-fetch, exact refund-ID polling, and raw webhook HMAC/stable event-ID binding | Provider retry, adapter, and webhook adversarial tests plus live order replay | Genuine Candidate 2 evaluation, authorization, asynchronous refund, and webhook delivery evidence absent |
