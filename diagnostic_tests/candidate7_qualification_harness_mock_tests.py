...
def test_incremental_evidence_written_before_and_after_attempt(tmp_path, monkeypatch):
    ...
    row = harness.run_attempt("D009", 1, tmp_path / "d009-1.json", provider_factory=GoodProvider, clock=FakeClock())
...
def test_first_429_stops_complete_qualification_immediately(tmp_path):
    ...
    class Provider:
        def _request(self, messages, attempt):
            calls["count"] += 1
            raise provider_error(429, retry=120.0, headers={"Retry-After": "120", "Authorization": "Bearer NEVER_SAVE"})
